...
async function optimizeForBrowserTabs(categories) {
  ...
  try {
    ...
    const primaryCategories = [
      {name: "💻办公", patterns: ["办公", "文档", "生产力", "效率", "管理", "会议", "邮箱", "协作"]},
      {name: "🔧技术", patterns: ["技术", "编程", "开发", "代码", "程序", "设计", "博客", "github", "stack", "AI", "模型", "训练", "工具", "教程", "学习", "教育", "课程", "培训", "知识", "教程", "学校", "研究", "科学"]},
      {name: "🎓学习", patterns: ["学习", "教育", "课程", "培训", "知识", "教程", "学校", "研究", "科学"]},
      {name: "🎥影视", patterns: ["影视", "电影", "电视剧", "综艺", "动漫", "纪录片", "电影推荐", "电视剧推荐", "综艺推荐", "动漫推荐", "纪录片推荐", "电影下载", "电视剧下载", "综艺下载", "动漫下载", "纪录片下载", "电影在线观看", "电视剧在线观看", "综艺在线观看", "动漫在线观看", "纪录片在线观看", "电影在线观看", "电视剧在线观看", "综艺在线观看", "动漫在线观看", "纪录片在线观看"]},
      {name: "🎮游戏", patterns: ["游戏", "游戏机", "游戏平台", "游戏攻略", "游戏资讯", "游戏论坛", "游戏社区", "游戏评测", "游戏推荐", "游戏下载", "游戏安装", "游戏更新", "游戏补丁", "游戏破解", "游戏修改", "游戏汉化", "游戏mod", "游戏存档", "游戏存档管理", "游戏存档备份", "游戏存档恢复", "游戏存档迁移", "游戏存档同步", "游戏存档分享", "游戏存档云同步", "游戏存档云备份", "游戏存档云恢复", "游戏存档云迁移", "游戏存档云同步", "游戏存档云备份", "游戏存档云恢复", "游戏存档云迁移"]},
      {name: "👥社交", patterns: ["社交", "媒体", "新闻", "资讯", "论坛", "社区", "微博", "朋友", "交友"]},
      {name: "🛒购物", patterns: ["购物", "电商", "消费", "商城", "网购", "优惠", "团购", "店铺", "商品"]},
      {name: "🏠生活", patterns: ["生活", "服务", "餐饮", "美食", "旅游", "出行", "健康", "医疗", "银行", "天气", "地图", "导航", "天气预报", "天气查询"]}
    ];
...
function createTreeNode(node, level) {
  ...
  title.addEventListener('dblclick', (e) => {
    e.stopPropagation();
    startInlineEdit(node, title, titleContainer);
  });
  ...
  quickEditBtn.className = 'tree-action-btn quick-edit-btn';
  quickEditBtn.textContent = '✏️';
  quickEditBtn.title = '快速重命名';
  ...
  detailEditBtn.className = 'tree-action-btn detail-edit-btn';
  detailEditBtn.textContent = '⚙️';
  detailEditBtn.title = '详细编辑';
  detailEditBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    editBookmarkItem(node);
  });
  actions.appendChild(detailEditBtn);
...
function showBatchRenameModal() {
  const selectedArray = Array.from(selectedBookmarks);
  const renameContainer = document.getElementById('batch-rename-container');
  const renameList = document.getElementById('batch-rename-list');
  
  if (!renameContainer) {
    // 创建批量重命名模态框
    createBatchRenameModal();
    return showBatchRenameModal();
  }
  ...
  selectedArray.forEach(nodeId => {
    ...
    if (node) {
      ...
      renameItem.className = 'batch-rename-item';
      
      renameItem.innerHTML = `
        <div class="rename-item-info">
          <span class="item-icon">${node.url ? '🔖' : '📁'}</span>
          <span class="item-path">${getNodePath(node)}</span>
        </div>
        <div class="rename-input-group">
          <input type="text" class="batch-rename-input" value="${node.title || ''}" 
                 data-node-id="${node.id}" data-original-title="${node.title || ''}">
          <button type="button" class="reset-name-btn" title="重置原名">↺</button>
        </div>
      `;
      
      renameList.appendChild(renameItem);
    }
  });
  ...
  // 聚焦第一个输入框
  const firstInput = renameList.querySelector('.batch-rename-input');
  if (firstInput) {
    firstInput.focus();
    firstInput.select();
  }
  
  addLogEntry(`准备批量重命名 ${selectedArray.length} 个项目`, 'info');
}

// 创建批量重命名模态框
function createBatchRenameModal() {
  const modalHtml = `
    <div id="batch-rename-container" class="modal hidden">
      <div class="modal-content large-modal">
        <div class="modal-header">
          <h3>批量重命名</h3>
          <button type="button" id="batch-rename-close" class="modal-close">×</button>
        </div>
        <div class="modal-body">
          <div class="batch-rename-controls">
            <div class="rename-templates">
              <label>快速模板：</label>
              <button type="button" class="template-btn" data-template="prefix">添加前缀</button>
              <button type="button" class="template-btn" data-template="suffix">添加后缀</button>
              <button type="button" class="template-btn" data-template="replace">查找替换</button>
              <button type="button" class="template-btn" data-template="number">编号命名</button>
            </div>
          </div>
          <div id="batch-rename-list" class="batch-rename-list"></div>
        </div>
        <div class="modal-footer">
          <button type="button" id="cancel-batch-rename" class="apple-btn apple-btn-secondary">取消</button>
          <button type="button" id="confirm-batch-rename" class="apple-btn apple-btn-primary">确认重命名</button>
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('cancel-batch-rename').addEventListener('click', closeBatchRenameModal);
  document.getElementById('confirm-batch-rename').addEventListener('click', confirmBatchRename);
  
  // 模板按钮事件
  document.querySelectorAll('.template-btn').forEach(btn => {
    btn.addEventListener('click', applyRenameTemplate);
  });
  
  // 重置按钮事件委托
  document.getElementById('batch-rename-list').addEventListener('click', (e) => {
    if (e.target.classList.contains('reset-name-btn')) {
...
// 应用重命名模板
function applyRenameTemplate(e) {
  ...
  switch (template) {
    ...
    case 'replace':
      ...
      if (searchText) {
        const replaceText = prompt('请输入替换的文本:') || '';
        inputs.forEach(input => {
          input.value = input.value.replace(new RegExp(searchText, 'g'), replaceText);
        });
      }
      break;
      
    case 'number':
      const startNum = parseInt(prompt('请输入起始编号:') || '1');
      const pattern = prompt('请输入编号模式 (使用 {n} 表示编号):') || '{n}. ';
      inputs.forEach((input, index) => {
        const num = startNum + index;
        const numberedName = pattern.replace('{n}', num) + input.dataset.originalTitle;
        input.value = numberedName;
      });
      break;
  }
}
...
async function confirmBatchRename() {
  ...
  if (renameOperations.length === 0) {
    showStatus('没有需要重命名的项目', 'warning');
    return;
  }
  
  if (!confirm(`确定要重命名 ${renameOperations.length} 个项目吗？`)) {
    return;
  }
  
  try {
    showLoading(true);
    addLogEntry(`开始批量重命名 ${renameOperations.length} 个项目...`, 'info');
    ...
    for (const operation of renameOperations) {
      try {
        await new Promise((resolve, reject) => {
          chrome.bookmarks.update(operation.nodeId, { title: operation.newTitle }, (result) => {
            if (chrome.runtime.lastError) {
              reject(new Error(chrome.runtime.lastError.message));
            } else {
              resolve(result);
        ...
        successCount++;
        addLogEntry(`重命名成功: "${operation.originalTitle}" → "${operation.newTitle}"`, 'success');
        
      } catch (error) {
        failedCount++;
        addLogEntry(`重命名失败: "${operation.originalTitle}" - ${error.message}`, 'error');
      }
    }
    
    addLogEntry(`批量重命名完成: 成功 ${successCount} 个，失败 ${failedCount} 个`, 'success');
    showStatus(`批量重命名完成: 成功 ${successCount} 个，失败 ${failedCount} 个`, 'success');
    ...
    showStatus(`批量重命名失败: ${error.message}`, 'error');
  } finally {
    showLoading(false);
  }
}
...
function closeBatchRenameModal() {
  const container = document.getElementById('batch-rename-container');
  if (container) {
    container.classList.add('hidden');
  }
}
...
function handleDragStart(e) {
  ...
  draggedNodeData = {
    nodeId: draggedNode.dataset.nodeId,
    sourceParent: draggedNode.parentElement
  };
  
  // 设置拖拽数据
  e.dataTransfer.effectAllowed = 'move';
  e.dataTransfer.setData('text/html', draggedNode.outerHTML);
  
  // 添加拖拽样式
  draggedNode.classList.add('dragging');
  
  addLogEntry(`开始拖拽节点: ${draggedNodeData.nodeId}`, 'info');
}
...
async function handleDrop(e) {
  ...
  if (!draggedNode || !draggedNodeData || targetNode === draggedNode) {
    return;
  }
  
  try {
    showLoading(true);
    addLogEntry(`尝试将节点 ${draggedNodeData.nodeId} 移动到 ${targetNodeId}`, 'info');
    ...
    // 查找目标节点信息
    const targetNodeData = findNodeById(targetNodeId);
    
    if (targetNodeData) {
      // 如果目标是文件夹，移动到文件夹内
      if (!targetNodeData.url && targetNodeData.children) {
        targetParentId = targetNodeId;
        targetIndex = 0; // 放在文件夹顶部
      } else {
        // 如果目标是书签，插入到其位置
        targetParentId = targetNodeData.parentId;
        
        // 获取目标父文件夹的子节点，确定插入位置
        const parentNodeData = findNodeById(targetParentId);
        if (parentNodeData && parentNodeData.children) {
          targetIndex = parentNodeData.children.findIndex(child => child.id === targetNodeId) + 1;
        }
      }
    }
    
    // 执行移动操作
    await moveBookmarkWithOrder(draggedNodeData.nodeId, targetParentId, targetIndex);
    
    addLogEntry(`成功移动节点到新位置`, 'success');
    showStatus('拖拽移动成功', 'success');
    ...
    addLogEntry(`拖拽移动失败: ${error.message}`, 'error');
...
function cleanupDragState() {
  // 移除所有拖拽相关的样式
  document.querySelectorAll('.tree-node.dragging').forEach(node => {
...
// 带顺序的移动书签
async function moveBookmarkWithOrder(bookmarkId, newParentId, insertIndex = null) {
  return new Promise((resolve, reject) => {
    // 先移动到目标父文件夹
    chrome.bookmarks.move(bookmarkId, { parentId: newParentId }, (result) => {
      if (chrome.runtime.lastError) {
...
// 开始直接编辑
async function startInlineEdit(nodeData, titleElement, titleContainer) {
  // 如果已经有正在编辑的元素，先保存
  if (currentEditingElement) {
    await saveInlineEdit();
  }
  
  addLogEntry(`开始直接编辑: ${nodeData.title}`, 'info');
  
  // 创建编辑输入框
  const input = document.createElement('input');
  input.type = 'text';
  input.className = 'tree-title-input';
  input.value = nodeData.title || '';
  input.dataset.nodeId = nodeData.id;
  input.dataset.originalTitle = nodeData.title || '';
  
  // 保存引用
  currentEditingElement = {
    input: input,
    originalElement: titleElement,
...
async function saveInlineEdit() {
  ...
  const { input, originalElement, container, nodeData } = currentEditingElement;
  const newTitle = input.value.trim();
  
  if (!newTitle) {
    showStatus('标题不能为空', 'warning');
    input.focus();
    return;
  }
  
  if (newTitle === nodeData.title) {
    // 没有变化，直接取消编辑
    cancelInlineEdit();
    return;
  }
  ...
  try {
    // 更新书签标题
    await new Promise((resolve, reject) => {
      chrome.bookmarks.update(nodeData.id, { title: newTitle }, (result) => {
        if (chrome.runtime.lastError) {
          reject(new Error(chrome.runtime.lastError.message));
        } else {
          resolve(result);
    ...
    // 更新显示
    originalElement.textContent = newTitle;
    container.replaceChild(originalElement, input);
    
    addLogEntry(`成功重命名: "${nodeData.title}" → "${newTitle}"`, 'success');
...
function cancelInlineEdit() {
  ...
  const { input, originalElement, container } = currentEditingElement;
